package br.com.joaonelson.pokedex.adapter
...
import br.com.joaonelson.pokedex.util.changeTypeColor
        ...
        override fun performFiltering(constraint: CharSequence?): FilterResults {
            ...
            if (constraint == null || constraint.isEmpty()) {
                ...
                for (item in fullPokemonList) {
                    ...
                    if(item.type1.lowercase().contains(filterPattern)) {
                        filteredList.add(item)
                    }
                    if(item.type2?.lowercase()?.contains(filterPattern) == true) {
                        filteredList.add(item)
                    }
                }
            }
